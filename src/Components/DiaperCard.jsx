...
const DiaperCard = ({ imgSrc, imgHeight = 'h-36', imgWidth = 'w-1/2', title, quantityProp, startingTotal }) => {
  ...
  const fetchCurrentQuantity = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/diapers/${title}/quantity`);
  ...
  const handleSubmit = async () => {
    const endpoint = quantityProp === "1" ? 'http://localhost:3001/api/diapers' : 'http://localhost:3001/api/beer';
    const data = quantityProp === "1" ? { name, quantity: parseInt(quantity, 10), title } : { name, title };
  ...
  return (
    <div className="max-w-xs mx-auto bg-white shadow-md rounded-lg overflow-hidden border-2 border-gray-300">
      <img src={imgSrc} alt="Diaper" className={`${imgWidth} ${imgHeight} object-cover`} />
      <div className="p-2">
